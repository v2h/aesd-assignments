...
{
    // 1) Hard-coded username from autotest-validate.c
    const char *hardcoded_username = my_username();

    // 2) Username read from conf/username.txt (caller must free the buffer)
    char *conf_username = malloc_username_from_conf_file();

    // 3) The two must match
    TEST_ASSERT_EQUAL_STRING_MESSAGE(hardcoded_username, conf_username,
        "my_username() does not match the username in conf/username.txt");
